function [massType, massM, massFcn, massArgs, dMoptions] = ...
    llmass(FcnHandlesUsed,ode,t0,y0,options,extras)
%ODEMASS  Helper function for the mass matrix function in ODE solvers
%    LLMASS determines the type of the mass matrix, initializes massFcn to
%    the mass matrix function and creates a cell-array of extra input
%    arguments. LLMASS evaluates the mass matrix at(t0,y0).  
%
...
if FcnHandlesUsed     % function handles used    
  ...
  elseif isnumeric(Moption)
    ...
    switch lower(Mstdep)
      ...
      otherwise
        error(message('llint:llmass:MStateDependenceMassType'));    
    end      
  ...
  switch(mass)
    ...
    otherwise
      error(message('llint:llmass:InvalidMassProp', mass));
  end
  massFcn = ode;  
