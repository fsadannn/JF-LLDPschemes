function options = llset(varargin)
...
while i <= nargin
  ...
  if ~isempty(arg)                      % [] is a valid options argument
    if ~isa(arg,'struct')
      error(message('llint:llset:NoPropNameOrStruct', i));
    end
    for j = 1:m
...
if rem(nargin-i+1,2) ~= 0
  error(message('llint:llset:ArgNameValueMismatch'));
end
...
while i <= nargin
  ...
  if ~expectval
    if ~ischar(arg)
      error(message('llint:llset:NoPropName', i));
    end

    lowArg = lower(arg);
    j = strmatch(lowArg,names);
    if isempty(j)                       % if no matches
      error(message('llint:llset:InvalidPropName', arg));
    elseif length(j) > 1                % if more than one match
      ...
      if length(k) == 1
        ...
        for k = j(2:length(j))'
            ...
            error(message('llint:llset:AmbiguousPropName',arg,matches));
      end
    end
...
if expectval
  error(message('llint:llset:NoValueForProp', arg));
end
