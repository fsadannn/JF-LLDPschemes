function [phi, err, m, nexpo,breakdown,nfeval,padepq, proportion, jorder,h] = phi1LLDP1(func, b, h, hmin, t, y, normy, m, ...
...
% idelta = 1/delta;
proportion = log(delta)/log(h);
% korder = proportion+1;
if proportion<4
    jorder = 2;
%     korder = proportion*2 + 1;
%     idelta=1/(2*delta);
end
...
%     freejf = @(vector) (func(t,y+(delta*hsubspace).*(vector))-func(t,y))/(delta);
    freejf = @(vector) idelta.*(func(t,y+delta.*(vector))-b);
else
    idelta=1/(2*delta);
    % order 2
%     h2delta = h/(2*delta);
%     freejf = @(vector)FreeJ_f_w(2,func,y,vector,t,delta,h2delta);
     freejf = @(vector) idelta.*(func(t,y+delta.*vector)-func(t,y-delta.*vector));
end
...
%      M190 = expm64v42_complex((h*cte).*H,pd,s);
...
%     err_rel=norm(((hk*M1(m,m+3)*beta/h).*avm1dot)./(atol+rtol.*abs(y)));
    ...
    err_rel_c = C*M1(m,m+3);
    err_rel=abs(err_rel_c);
