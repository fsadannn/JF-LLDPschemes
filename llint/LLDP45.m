function varargout = LLDP45(ode,odeFx,tspan,y0,options,varargin)

%   LLDP45 Locally Linearized Runge-Kutta method of Dormand and Prince with exact Jacobian matrix
%   [TOUT,YOUT] = LLDP45(ODEFUN,ODEJAC,TSPAN,Y0) with TSPAN = [T0 TFINAL] integrates
...
%   [T0 T1 ... TFINAL].
%
%   [TOUT,YOUT] = LLDP45(ODEFUN,ODEJAC,TSPAN,Y0,OPTIONS) solves as above with default
...
%   'none'.
%
%   [TOUT,YOUT,TE,YE,IE] = LLDP45(ODEFUN,ODEJAC,TSPAN,Y0,OPTIONS) with the 'Events'
...
%
%   SOL = LLDP45(ODEFUN,ODEJAC,[T0 TFINAL],Y0...) returns a structure that can be
...
%   Naranjo-Noda F.S., Jimenez J.C., Locally Linearized Runge-Kutta method of Dormand and Prince
%   for large systems of initial value problems, J. Comput. Physics 426 (2021) 109946.
%   Copyright (c) 2021, Frank S. Naranjo-Noda
  ...
  nfevals = nfevals + 1;
  nJevals = nJevals + 1;
  isAdaptiveJacActive = ~isinf(jacgap);
  ...
  while ~done
    ...
    if  nsteps ~= currentJstep
         currentJ = false;
    end
    if isAdaptiveJacActive
      if ~currentJ
          fxf = Fx*f1;
          jacerr = norm(abs(f1-fprev-fxf) ./ max(abs(ffold),threshold),inf);
          if jacerr > jacgap
                Fx = odeFxcn_main(t,y);
                nnorm=norm(Fx,'inf');
                currentJ=true;
    ...
    while true
       ...
       if ~nofailed && ~currentJ && isAdaptiveJacActive
          Fx = odeFxcn_main(t,y);
          nnorm=norm(Fx,'inf');
          currentJ=true;
          nJevals = nJevals + 1;
      ...
      if nofailed || ~reuse
           [phi,kerror,kdim,nexpcont,breakdown, padepq,h] = phi1LLDP_hJ_f_gamma(Fx,f1,h,hmin,y,nhC,kdim,...
    ...
    if output_ty || haveOutputFcn
      switch outputAt
       ...
       case 3      % computed points, with refinement
        ...
        nout_new = refine;
        tout_new = [tref, tnew];
        yntrp45 = ntrpLLRK45_Krilov_split(tref,t,y,h,[zeros(length(f2),1) f2 f3 f4 f5 f6 f7],idxNonNegative,Fx,f1,kdmax);
  ...
  if nargout == 1
      ...
      solver_output{1}.stats.krilov_reject = 0;
  end
