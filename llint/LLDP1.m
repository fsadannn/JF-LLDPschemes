...
%   [T0 T1 ... TFINAL].     
%   
%   [TOUT,YOUT] = LLDP1(ODEFUN,TSPAN,Y0,OPTIONS) solves as above with default
...
%   'none'.
%
%   [TOUT,YOUT,TE,YE,IE] = LLDP1(ODEFUN,TSPAN,Y0,OPTIONS) with the 'Events'
...
%
%   SOL = LLDP1(ODEFUN,[T0 TFINAL],Y0...) returns a structure that can be
...
while ~done
  ...
  while true
     ...
     if nofailed
         [phi,kerror,kdim,nexpcont,breakdown,knfeval,padepq,alpha,eta,h] = phi1LLDP1(odeFcn_main,f1,h,hmin,t,y,ny,kdim,...
     ...
     eta_v_1_cont = eta_v_1_cont + rem(eta,2);
    ...
    if alpha_2 <3 && (alpha_2+1)<alpha*eta
    ...
    if err > rtol                       % Failed step
      ...
      alphak = min(alpha,alphak);
      alphak_max = max(alpha,alphak_max);
...
if nargout == 1
    ...
    etak_max = max([1,(alphak_max<4)*2, (alphak<4)*2]);
    etak = max([min([(alphak_max<4)*2, (alphak<4)*2]),1]);
    solver_output{1}.stats.korder =  min([korder_rk,korder_krylov,5]);
    solver_output{1}.stats.korder_krylov = korder_krylov;
    solver_output{1}.stats.eta = etak;
    solver_output{1}.stats.alpha = alphak;
    ...
    solver_output{1}.stats.eta_max = etak_max;
    solver_output{1}.stats.alpha_max = alphak_max;
    solver_output{1}.stats.korder_rk_max = korder_rk_max;
    solver_output{1}.stats.alpha2_max = alpha2_max;
    solver_output{1}.stats.eta_v_1_cont = eta_v_1_cont;
